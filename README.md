...
프로젝트 루트에 `.env` 파일을 생성하세요 (`.gitignore`에 포함돼 커밋되지 않습니다):
...
- [ ] 기상청(KMA) API 연동으로 날씨 예보 기반 예측 추가
- [ ] 단말 모듈 소비 전력 측정 및 배터리 수명 계산
- [ ] 실제 LoRa → AWS 현장 검증 (단말 → 게이트웨이 → DB → AI → Web 전체 흐름)
- [ ] 다중 설치 지역 지원을 위한 노드 설정 일반화
