...
| 표시 | 빨간 디스플레이(현재 온도) / 파란 디스플레이(현재 습도) |
...
실험 당시 온도 컨트롤러 수치는 약 7°C 까지 내려가는 것을 확인하였다.

### 결론

펠티어 방식의 소형 냉각 챔버로 영하 환경을 만드는 것은 충분한 단열재 적용과 고출력 펠티어 소자 또는 다단 스택 구성이 필요하다고 판단했다. 해당 챔버를 전시회전까지 완성하고 시연을 완벽하게 하는 것을 목표로 한다. 또한 전시회 전에 챔버가 완성 된다면 통해 AI 학습 데이터를 만들어 학습 시킬 예정이다.
